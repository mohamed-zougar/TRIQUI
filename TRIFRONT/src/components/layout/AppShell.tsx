...
export function AppShell({ user, children, showNav = true }: AppShellProps) {
  ...
  return (
    <div className="min-h-screen bg-[var(--color-surface-muted)] pb-20 lg:pb-0">
      ...
      <div className="sticky top-0 z-40 w-full bg-[var(--color-brand-500)] px-4 py-3 shadow-md lg:hidden">
      ...
      {showNav ? <BottomNav pathname={pathname} user={user} /> : null}
    </div>
  );
}

function BottomNav({ pathname, user }: { pathname: string; user: AuthSummary | null }) {
  ...
  return (
    <>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-30 border-t border-[var(--color-border)] bg-white pb-[max(env(safe-area-inset-bottom),0.25rem)] lg:hidden">
        <div className={cn("relative h-16 items-stretch grid", user?.role === "admin" ? "grid-cols-6" : "grid-cols-5")}>
          ...
          {user?.role === "admin" ? (
            <Link
              ...
              className={cn(
                "flex h-full flex-col items-center justify-center gap-1 text-[11px] font-medium",
                pathname.startsWith("/admin") ? "text-[var(--color-brand-500)]" : "text-[var(--color-fg-muted)]"
              )}
            >
              <Shield size={20} />
              <span>Admin</span>
